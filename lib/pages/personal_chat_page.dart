...
class _PersonalChatPageState extends State<PersonalChatPage> {
  ...
  setUpReferences()async{
    ...
    await getCurrentUser().then((curruid){
      ...
      friendRef = FirebaseDatabase.instance.reference().child('chat').child(widget.user.uid).child(currentUserID);
      myRef = FirebaseDatabase.instance.reference().child('chat').child(currentUserID).child(widget.user.uid);
  ...
  void onFocusChange() {
    if (focusNode.hasFocus) {
      setState(() {
        listScrollController.animateTo(listScrollController.position.minScrollExtent, curve: Curves.easeOut, duration: Duration(microseconds: 1));
      });
    }
  }
      ...
      await myRef.child(msgKey).child("id").set(key).then((v) async{
        await friendRef.child(msgKey).child("id").set(key).then((v) async{
          var notification = OSCreateNotification(
              ...
              content: messageText,
              heading: widget.user.name,
              additionalData: <String,String>{"userid":currentUserID},
              bigPicture: imageUrl,
          );
          await OneSignal.shared.postNotification(notification).then((response){
            setState(() {
              _isComposingMessage = false;
            });
  ...
  void _deleteMessage(Message _msg,DataSnapshot snapshot) async{
    ...
    if (_msg.imageURL.isNotEmpty){
      StorageReference storageReference = await FirebaseStorage.instance.getReferenceFromUrl(_msg.imageURL);
  ...
  Widget build(BuildContext context) {
    ...
    _messageBox(){
      return Container(
        ...
        Padding(padding: EdgeInsets.all(10.0),child:Center(child: CircularProgressIndicator()))
        : Row(
          ...
          children: <Widget>[
            Material(
              ...
              child: IconButton(
                color: Colors.transparent,
                icon: Icon(Icons.camera_alt,size: _width/12, color: Colors.white70),
                onPressed: () {
                  _sendImage();
                },
              ),
              ...
              child: Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    child: Icon(
                      Icons.send, size:_width/15, color: Theme.of(context).accentColor,
                    ),
                  ),
                ),
              ),
        ...
        title: InkWell(
          ...
          onTap: (){
            Navigator.of(context).push(
                MaterialPageRoute(builder: (BuildContext context) => ProfilePage(widget.user))
            );
          },
          child: Container(
            height: _height,
            ...
            Flexible(
              child: FirebaseAnimatedList(
                ...
                itemBuilder: (BuildContext context, DataSnapshot snapshot,
                  ...
                  Message _msg = Message(time:"",text:"",sender:"",imageURL: "");
                  return snapshot != null ?
                  StreamBuilder(
                    stream: msgRef.child(snapshot.value['id']).onValue,
                    builder: (context, snap){
                      if(snap.hasData && !snap.hasError && snap.data.snapshot.value!=null) {
                        _msg.sender = snap.data.snapshot.value['sender'].toString();
                        _msg.text = snap.data.snapshot.value['text'].toString();
                        _msg.time = snap.data.snapshot.value['time'].toString();
                        snap.data.snapshot.value['imageUrl'] != null ? _msg.imageURL = snap.data.snapshot.value['imageUrl'].toString()  : _msg.imageURL="";
                      }
                      return InkWell(
                        onLongPress: (){
                          confirmModalSheet(context, 'Delete message', () async{
